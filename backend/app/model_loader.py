...
class LightweightSentimentModel:
    ...
    def _verify_model_integrity(self, model_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info("Checking classifier path: %s", classifier_path)
            logger.info("Checking vectorizer path: %s", vectorizer_path)
            logger.info("Classifier exists: %s", classifier_path.exists())
            logger.info("Vectorizer exists: %s", vectorizer_path.exists())

            if not classifier_path.exists() or not vectorizer_path.exists():
                logger.error("Model files not found. Classifier: %s, Vectorizer: %s",
                             classifier_path.exists(), vectorizer_path.exists())
                return False

            classifier_hash = self._calculate_file_sha256(classifier_path)
            vectorizer_hash = self._calculate_file_sha256(vectorizer_path)

            logger.info("Calculated classifier SHA256: %s", classifier_hash)
            logger.info("Expected classifier SHA256: %s", model_config['classifier_sha256'])
            logger.info("Calculated vectorizer SHA256: %s", vectorizer_hash)
            logger.info("Expected vectorizer SHA256: %s", model_config['vectorizer_sha256'])
            ...
            if not classifier_valid:
                logger.warning("Classifier SHA256 mismatch. Expected: %s, Got: %s",
                               model_config['classifier_sha256'], classifier_hash)
            if not vectorizer_valid:
                logger.warning("Vectorizer SHA256 mismatch. Expected: %s, Got: %s",
                               model_config['vectorizer_sha256'], vectorizer_hash)

            return classifier_valid and vectorizer_valid

        except Exception as e:
            logger.error("Error verifying model integrity: %s", e)
            return False
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            logger.info("Ultra model verification result: %s", verification_result)
    ...
    def _load_ultra_model(self, model_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("Error loading ultra model: %s", e)
            return False
    ...
    def _load_numpy_model(self, weights_path: Path, metadata_path: Path) -> bool:
        ...
        try:
            ...
            logger.error("Error loading numpy model: %s", str(e))
            return False
        ...
        try:
            ...
            for model_name in model_names:
                ...
                if all(path.exists() for path in [vectorizer_path, classifier_path, metadata_path]):
                    logger.info("Loading joblib model: %s", model_name)
            ...
            logger.error("Joblib fallback failed: %s", str(e))
            return False
    ...
    def predict(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Prediction: %s, Confidence: %.3f", sentiment_label, confidence_score)
